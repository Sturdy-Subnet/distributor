...
async def create_test_database(db_path, alpha_price_in_tao):
    ...
    # Create batch IDs for test transfers
    batch_id_1 = str(uuid.uuid4())
    batch_id_2 = str(uuid.uuid4())
    batch_id_3 = str(uuid.uuid4())
    ...
    # Create corresponding moves entries (completed status so transfers can be processed)
    test_moves = [
        (batch_id_1, "completed", test_coldkey, test_hotkey, test_hotkey, 1.0),
        (batch_id_2, "completed", test_coldkey, test_hotkey, test_hotkey, 1.0),
        (batch_id_3, "completed", test_coldkey, test_hotkey, test_hotkey, 1.0),
    ]

    # Insert test moves first
    for move in test_moves:
        conn.execute(
            "INSERT INTO moves (batch_id, status, origin_coldkey, origin_hotkey, destination_hotkey, amount) VALUES (?, ?, ?, ?, ?, ?)",
            move,
        )
...
async def test_merged_transfers_can_be_processed():
    """Test that merged transfers can be processed by run_pending_transfers"""
    # Import the functions we want to test
    # pylint: disable=import-error
    from distributor import merge_tiny_transfers, run_pending_transfers
    ...
    try:
        ...
        # Mock distribution_subtensor for transfer operations
        mock_distribution_subtensor = MagicMock()
        mock_distribution_subtensor.transfer_stake = AsyncMock(return_value=True)

        # First merge the tiny transfers
        print("Step 1: Merging tiny transfers...")
        await merge_tiny_transfers(db_path, mock_wallet, mock_subtensor)

        # Check that we have pending transfers that can be processed
        conn = sqlite3.connect(db_path)
        cursor = conn.execute(
            """
        ...
        processable_transfers = cursor.fetchone()[0]
        print(f"Processable transfers after merge: {processable_transfers}")

        # Verify that we have transfers that can be processed
        assert processable_transfers > 0, "No transfers can be processed after merge"

        # Now try to process the pending transfers
        print("Step 2: Processing pending transfers...")
        try:
            # We expect this to work without errors now
            await run_pending_transfers(
                db_path, mock_wallet, mock_distribution_subtensor, mock_subtensor
            )
            print("✅ Pending transfers processed successfully!")
        except Exception as e:
            print(f"❌ Error processing pending transfers: {e}")
            raise

        # Verify that transfer_stake was called
        assert mock_distribution_subtensor.transfer_stake.called, (
            "transfer_stake should have been called"
        )
        print(
            f"✅ transfer_stake was called {mock_distribution_subtensor.transfer_stake.call_count} times"
        )

        conn.close()
        print("\n✅ Test passed! Merged transfers can be processed successfully.")
...
if __name__ == "__main__":
    ...
    async def run_all_tests():
        print("=== Running Test 1: Basic Merge Functionality ===")
        await test_merge_functionality()

        print("\n=== Running Test 2: Merged Transfers Processing ===")
        await test_merged_transfers_can_be_processed()
